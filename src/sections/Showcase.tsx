    ...
    <section id="showreel" ref={ref} className="relative h-[300vh] bg-black">
      <div className="sticky top-0 h-[100svh] overflow-hidden">
        <div data-showreel-media className="absolute inset-0 origin-center will-change-transform">
          {videoFailed ? (
            <Image src="/images/panther-still.jpg" alt="Crni panter gleda pravo u kameru" fill sizes="100vw" className="object-cover" />
          ) : (
            <video ref={videoRef} muted playsInline preload="auto" poster="/images/panther-poster.jpg" aria-hidden className="absolute left-1/2 top-1/2 h-auto w-[200vw] max-w-none -translate-x-1/2 -translate-y-1/2 md:inset-0 md:left-0 md:top-0 md:h-full md:w-full md:translate-x-0 md:translate-y-0 md:object-cover" />
          )}
        </div>
        <div data-showreel-blackout className="absolute inset-0 bg-black" />
        <div data-showreel-veil className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/10 to-black/35 opacity-0" />
        <div data-showreel-copy className="absolute inset-x-0 bottom-0 px-5 pb-[10svh] opacity-0 md:px-10 md:pb-[12svh]">
